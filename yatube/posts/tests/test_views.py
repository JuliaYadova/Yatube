...
class PostPagesTests(TestCase):
    ...
    def setUpClass(cls):
        ...
        for i in range(6):
            Post.objects.create(author=cls.user_two,
    ...
    def test_index_page_show_correct_context(self):
        ...
        if (Post.objects.count()
            - settings.PER_PAGE_COUNT
           < settings.PER_PAGE_COUNT):
            posts_for_two_page = (Post.objects.count()
                                  - settings.PER_PAGE_COUNT
                                  )
        else:
    ...
    def test_group_list_page_show_correct_context(self):
        ...
        if (Post.objects.filter(group=PostPagesTests.group).count()
            - settings.PER_PAGE_COUNT
           < settings.PER_PAGE_COUNT):
            posts_for_two_page = (
    ...
    def test_profile_page_show_correct_context(self):
        ...
        if (Post.objects.filter(author=PostPagesTests.user).count()
            - settings.PER_PAGE_COUNT
           < settings.PER_PAGE_COUNT):
            posts_for_two_page = (
    ...
    def test_new_post_create_correct(self):
        ...
        response = self.auth_client.get(reverse(
            'posts:group_posts',
            kwargs={'slug': f'{PostPagesTests.group_two.slug}'}
        )
        )
        first_obj = response.context['page_obj'][0]
        obj_text_0 = first_obj.text
        ...
        response = self.auth_client.get(reverse(
            'posts:profile',
            kwargs={'username': f'{PostPagesTests.user_two}'}
        )
        )
        first_obj = response.context['page_obj'][0]
        obj_text_0 = first_obj.text
    ...
    def test_index_page_include_cash(self):
        ...
        new_post = Post.objects.create(author=PostPagesTests.user,
    ...
    def test_follow_page_(self):
        ...
        self.assertTrue(Follow.objects.filter(
            ...
            author=PostPagesTests.user_two).exists()
        )
    ...
    def test_follow_index_page_(self):
        ...
        new_post = Post.objects.create(author=PostPagesTests.user_two,
